...
public class ImportScanBarcodeService {
    ...
    @Transactional
    public ImportItemsResponse importItemsByScan(ImportScanBarcodeRequest request) {
        ...
        for (ScanBarcodeDTO dto : request.getScannedItems()) {
            ...
            // Tìm các box phù hợp trong bin có thể chứa sku này
            ...
            while (quantityRemaining > 0) {
                ...
                // Nếu không còn box chứa được -> tạo box mới
                if (targetBox == null) {
                    ...
                    targetBox = Box.builder()
                            ...
                            .bin(targetBin)
                            .sku(sku)
                            .capacity(1000) // hoặc giá trị mặc định
                            .usedCapacity(0)
                            .isDeleted(false)
                            .build();
                ...
                // Tính số item có thể đưa vào box này
